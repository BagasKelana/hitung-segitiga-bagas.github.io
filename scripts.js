function katika(){const buttonMain = document.querySelector('.btn-section');
 ...
 document.querySelector('.btn-hasil2').addEventListener('click', (n) => {
    if(n.target.className === 'hasil'){
        if(validation(paramValidasi)) {
            document.querySelector('.output-keliling').innerHTML = `
            <p>K = S1 x S2 x S3</p>
            <p>K = ${inputKeliling1.value} x ${inputKeliling2.value} x ${inputKeliling3.value}</p>
            <p>L=${inputKeliling1.value*inputKeliling2.value*inputKeliling3.value} </p>`
        }
